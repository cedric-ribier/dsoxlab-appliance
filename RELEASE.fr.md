...
Les dépôts propres de Debian ne fournissent pas VirtualBox de façon
fiable (retiré à cause des licences du module kernel non-libre) —
utilise plutôt le dépôt officiel Oracle, qui fonctionne à l'identique
sous Ubuntu :
...
Empreinte de clé à vérifier si tu veux confirmer avant de faire
confiance : `B9F8 D658 297A F3EF C18D  5CDF A2F6 83C5 2980 AECF`
(Oracle Corporation, clé de signature d'archive VirtualBox).

Si `apt update` rapporte des erreurs `BADSIG` ensuite (signatures en
cache périmées d'une tentative précédente) :
```bash
sudo apt-get clean
...
Ajoute ton utilisateur au groupe VirtualBox :
